...
_expand_alias() {
    ...
    if (( $beg == $end )) ; then
        ...
        ##
        ##  warn: `_split_cmd_line` drops redirections and assignments only in
        ##  `COMP_WORDS` not in `COMP_LINE`; this may cause a mismatch between
        ##  these two variables; this seems ok for now; if a bug is found from
        ##  this mismatch in the future, consider dropping them in `COMP_LINE`
        ##  as well;
...
##  alternatively, uncomment this line to complete all your aliases;
